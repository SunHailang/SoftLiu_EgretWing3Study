        ...
        var TrailBatcher = (function () {
            ...
            TrailBatcher.prototype._rebuildMeshData = function (now) {
                ...
                for (var i = 0; i < count; ++i) {
                    ...
                    var time = (now - p.timeCreated) / (comp.time * 1000);
                    ...
                    switch (comp.textureMode) {
                        case trail.TrailTextureMode.Stretch:
                            ratioSum = this._buildMeshForTextureStretch(vertex1, vertex2, color, i, ratioSum);
                            break;
            ...
            TrailBatcher.prototype._buildMeshForTextureStretch = function (vertex1, vertex2, color, index, ratioSum) {
                var i = index * 6;
                ...
                // 同样的颜色值
                i = index * 8;
                this._colors[i + 0] = color.r;
                this._colors[i + 1] = color.g;
                this._colors[i + 2] = color.b;
                this._colors[i + 3] = color.a;
                this._colors[i + 4] = color.r;
                this._colors[i + 5] = color.g;
                this._colors[i + 6] = color.b;
                this._colors[i + 7] = color.a;
                // 两点的 uv 值
            ...
            TrailBatcher.prototype._getColorSample = function(comp, time) {
                ...
                if (comp.color.length && comp.color.length > 0) {
                    time = time < 0?0:time;
                    time = time > 1?1:time;

                    var colorTime = time * (comp.color.length - 1);
                    var min = Math.floor(colorTime);
                    var max = Math.ceil(colorTime);
                    if(min >= max){
                        color.copy( comp.color[min]);
                    }else{
                        var lerp = (colorTime - min) / (max - min)
                        color.lerp(comp.color[min], comp.color[max], lerp);
                    }
                } else {
                    
                    color.copy( comp.color);
                }
            ...
            TrailBatcher.prototype._getWidthSample = function (comp, time) {
                ...
                var width = 0;
                if (comp.width.length && comp.width.length > 0) {
                    time = time < 0?0:time;
                    time = time > 1?1:time;

                    var widthTime = time * (comp.width.length - 1);
                    var min = Math.floor(widthTime);
                    var max = Math.ceil(widthTime);
                    if(min >= max){
                        width = comp.width[min];
                    }else{
                        var lerp = (widthTime - min) / (max - min)
                        width = comp.width[min] + lerp * (comp.width[max] - comp.width[min]);
                    }
                } else {
            ...
            TrailBatcher.prototype._composeMesh = function () {
                ...
                buff = this._mesh.getAttributes("COLOR_0");
                
                if (buff) { buff.fill(0.0); }
                this._mesh.setAttributes("COLOR_0", this._colors);
            ...
            TrailBatcher.prototype._createMesh = function () {
                this._mesh = egret3d.Mesh.create((this._maxFragmentCount - 1) * 4, (this._maxFragmentCount - 1) * 6, ["POSITION","TEXCOORD_0", "COLOR_0"]);
